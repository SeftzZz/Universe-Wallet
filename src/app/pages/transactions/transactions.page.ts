...
export class TransactionsPage implements OnInit {
  ...
  async ngOnInit() {
    ...
    if (saved) {
      ...
      this.loadWalletTrades();   // ✅ dipanggil setelah address siap
    }
  }
  ...
  async loadWalletTrades() {
    ...
    try {
      ...
      const res: any = await this.http
        .get(`${environment.apiUrl}/wallet/trades?wallet=${this.userAddress}`)
        .toPromise();
  ...
  async ionViewWillEnter() {
    ...
    if (this.userAddress) {
      await this.loadWalletTrades(); // ✅ reload otomatis saat kembali ke halaman
    }
  }
